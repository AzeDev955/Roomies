...
export default function CaseroTablonScreen() {
  const [contexto, setContexto] = useState<ContextoCasero>(null);
  ...
  const cargarContexto = async () => {
    setLoadingCtx(true);
    try {
      const { data: perfil } = await api.get<{ id: number }>('/auth/me');
      const { data: viviendas } = await api.get<{ id: number }[]>('/viviendas');
      if (viviendas.length > 0) {
        setContexto({ viviendaId: viviendas[0].id, usuarioId: perfil.id });
      } else {
        setContexto(null);
      }
    } catch {
    ...
    return (
      <View style={[styles.container, { justifyContent: 'center' }]}>
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptyTitulo}>Primero añade una vivienda</Text>
          <Text style={styles.emptySubtitulo}>
            El tablón aparecerá aquí en cuanto tengas una vivienda creada.
          </Text>
        </View>
      </View>
    );
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              ...
              <Text style={styles.emptySubtitulo}>
                Sé el primero en publicar algo para tu vivienda.
              </Text>
            </View>
          }
        />
